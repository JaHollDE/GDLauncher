...
const main = async () => {
  ...
  try {
    const { data: releasesList } = await axios.default.get(
      `https://api.github.com/repos/JaHollDE/JaHollDELauncher/releases`,
      {
        headers: {
      ...
      for (const element in rawChangeLog) {
        if (rawChangeLog[element].length) {
          ...
          for (const e of rawChangeLog[element]) {
            ...
            const advanced =
              e?.advanced?.cm &&
              ` ([${e?.advanced?.cm}](https://github.com/JaHollDE/JaHollDELauncher/commit/${e?.advanced?.cm})` +
                `${
                  prSplit
                    ? ` | [#${e?.advanced.pr}](https://github.com/JaHollDE/JaHollDELauncher/pull/${prSplit[0]}` +
